...
| custom.filters          | 文档筛选条件，目前仅支持设置 1 个条件                                                                            | object[] | 否   |
| custom.filters[].title  | 筛选条件名称                                                                                                     | string   | 否   |
| custom.filters[].type   | 筛选条件类型，目前仅支持`group`，多级分类筛选                                                                    | string   | 否   |
| custom.filters[].data   | 多级分类筛选的列 id 列表                                                                                         | string[] | 否   |
| custom.operations       | 支持的默认操作，可以关闭指定操作（默认不关闭），包括：`create, edit, remove, editMany, removeMany, transferMany` | object   | 否   |
| custom.removeConstraint | 删除文档的限制条件。符合 MongoDB 的查询条件。                                                                    | object   | 否   |
