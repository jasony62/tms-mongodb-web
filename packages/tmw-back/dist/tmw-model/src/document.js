...
class Document extends base_1.default {
    ...
    async remove(existCl, id) {
        ...
        if (result[0] && result[1]["insert"]) {
            ...
            const { targetSysCl, targetQuery } = await this.getUnRepeatSQ(removeSysCl, removeQuery, dbSysName, clSysName, keys);
    ...
    async removeMany(existCl, query) {
        ...
        if (existCl.usage !== 1) {
            ...
            if (result[0] && result[1]["insert"]) {
                ...
                const clSysName = result[1]["clSysName"];
                const keys = result[1]["keys"];
                const { targetSysCl, targetQuery } = await this.getUnRepeatSQ(sysCl, query, dbSysName, clSysName, keys);
                targetSysCl.deleteMany(targetQuery);
            }
            ...
            for (const entry of priIdsByCl) {
                ...
                if (result[0]["flag"] && "insert") {
                    ...
                    const { targetSysCl, targetQuery } = await this.getUnRepeatSQ(sysCl, { _id: { $in: ids } }, dbSysName, clSysName, keys);
                    targetSysCl.deleteMany(targetQuery);
                }
    ...
    async list(existCl, filter = {}, orderBy = {}, page = 0, size = 0, like = true) {
    ...
    async dataActionLog(oDatas, operate_type, dbname, clname, operate_after_dbname = '', operate_after_clname = '', operate_before_data = null, client_info = null) {
