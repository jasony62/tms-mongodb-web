...
class ReplicaBase extends base_1.default {
    constructor(...args) {
        ...
        this.modelReplicaMap = new tmw_model_1.ReplicaMap(this.mongoClient);
    }
    async tmsBeforeEach() {
    ...
    async list() {
        ...
        if (primary && typeof primary === 'object') {
            ...
            if (success !== true)
                return new ResultFault(`主集合-${priDbOrCause}`);
        ...
        if (secondary && typeof secondary === 'object') {
            ...
            if (success !== true)
                return new ResultFault(`从集合-${secDbOrCause}`);
    ...
    async create() {
        ...
        if (passed !== true)
            return new ResultFault(causeOrBefore);
        if (causeOrBefore) {
            let { primary, secondary } = this.request.body;
            return new ResultFault(`不允许重复建立集合复制关系[primary.db=${primary.db}][primary.cl=${primary.cl}][secondary.db=${secondary.db}][secondary.cl=${secondary.cl}]`);
        }
        const replicaMap = {
