...
class CollectionBase extends base_1.default {
    ...
    async byName() {
        ...
        return new tms_koa_1.ResultData(existCl);
    }
    async list() {
        ...
        if (typeof skip === 'number') {
            ...
            return new tms_koa_1.ResultData({ collections: tmwCls, total });
        }
        return new tms_koa_1.ResultData(tmwCls);
    }
    async create() {
        ...
        if (!info.name)
            return new tms_koa_1.ResultFault('集合名称不允许为空');
    ...
    async update() {
        ...
        if (newClName) {
            ...
            if (otherCl)
                return new tms_koa_1.ResultFault(`数据库[name=${this["reqDb"].name}]中，已存在同名集合[name=${newClName}]`);
        }
    ...
    async remove() {
        ...
        if ((db.sysname === 'admin' && clName === 'system.version') ||
            ...
            return new tms_koa_1.ResultFault(`系统自带集合[${clName}]，不能删除`);
        if (usage !== undefined) {
            const [flag] = await this["rpHelper"].byId(existCl);
            if (flag)
                return new tms_koa_1.ResultFault(`该集合存在关联关系不允许删除`);
        }
        ...
        return this["clMongoObj"]
            ...
            .catch((err) => new tms_koa_1.ResultFault(err.message));
    }
}
