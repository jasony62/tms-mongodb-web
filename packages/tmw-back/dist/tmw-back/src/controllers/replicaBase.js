"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.ReplicaBase = void 0;
const log4js = require("log4js");
const logger = log4js.getLogger('tms-mongodb-web');
const tms_koa_1 = require("tms-koa");
const base_1 = require("./base");
const replicaHelper_1 = require("./replicaHelper");
const replicaMap_1 = require("../../../tmw-model/src/replicaMap");
class ReplicaBase extends base_1.default {
    constructor(...args) {
        super(...args);
        this["replicaHelper"] = new replicaHelper_1.default(this);
        this["modelReplicaMap"] = new replicaMap_1.default(this["mongoClient"]);
    }
    async tmsBeforeEach() {
        let result = await super.tmsBeforeEach();
        if (true !== result)
            return result;
        this["clMongoObj"] = this["replicaHelper"].clMongoObj;
        this["clReplicaMap"] = this["replicaHelper"].clReplicaMap;
        return true;
    }
    async list() {
        const { primary, secondary } = this["request"].body;
        const query = {};
        if (primary && typeof primary === 'object') {
            const [success, priDbOrCause, priCl] = await this["replicaHelper"].findDbAndCl(primary);
            if (success !== true)
                return new tms_koa_1.ResultFault(`主集合-${priDbOrCause}`);
            query['primary.db'] = priDbOrCause.sysname;
            query['primary.cl'] = priCl.sysname;
        }
        if (secondary && typeof secondary === 'object') {
            const [success, secDbOrCause, secCl] = await this["replicaHelper"].findDbAndCl(secondary);
            if (success !== true)
                return new tms_koa_1.ResultFault(`从集合-${secDbOrCause}`);
            query['secondary.db'] = secDbOrCause.sysname;
            query['secondary.cl'] = secCl.sysname;
        }
        const options = {
            projection: { type: 0 },
            sort: { top: -1 },
        };
        let { skip, limit } = this["replicaHelper"].requestPage();
        if (typeof skip === 'number') {
            options.skip = skip;
            options.limit = limit;
        }
        const maps = await this["clReplicaMap"].find(query, options).toArray();
        for (let i = 0, map; i < maps.length; i++) {
            map = maps[i];
            let { _id, primary, secondary } = map;
            let ts = _id.getTimestamp() * 1;
            map.createTime = ts;
            let priDb = await this["clMongoObj"].findOne({
                sysname: primary.db,
                type: 'database',
            });
            let priCl = await this["clMongoObj"].findOne({
                'db.sysname': priDb.sysname,
                sysname: primary.cl,
                type: 'collection',
            });
            if (priCl) {
                primary.db = { name: priDb.name, title: priDb.title };
                primary.cl = { name: priCl.name, title: priCl.title };
            }
            let secDb = await this["clMongoObj"].findOne({
                sysname: secondary.db,
                type: 'database',
            });
            let secCl = await this["clMongoObj"].findOne({
                'db.sysname': secDb.sysname,
                sysname: secondary.cl,
                type: 'collection',
            });
            if (secCl) {
                secondary.db = { name: secDb.name, title: secDb.title };
                secondary.cl = { name: secCl.name, title: secCl.title };
            }
            delete map._id;
        }
        if (typeof skip === 'number') {
            let total = await this["clReplicaMap"].countDocuments(query);
            return new tms_koa_1.ResultData({ replicas: maps, total });
        }
        return new tms_koa_1.ResultData(maps);
    }
    async create() {
        const [passed, causeOrBefore, pri, sec] = await this["replicaHelper"].checkRequestReplicaMap();
        if (passed !== true)
            return new tms_koa_1.ResultFault(causeOrBefore);
        if (causeOrBefore) {
            let { primary, secondary } = this["request"].body;
            return new tms_koa_1.ResultFault(`不允许重复建立集合复制关系[primary.db=${primary.db}][primary.cl=${primary.cl}][secondary.db=${secondary.db}][secondary.cl=${secondary.cl}]`);
        }
        const replicaMap = {
            primary: { db: pri.db.sysname, cl: pri.cl.sysname },
            secondary: { db: sec.db.sysname, cl: sec.cl.sysname },
        };
        return this["clReplicaMap"]
            .insertOne(replicaMap)
            .then((result) => new tms_koa_1.ResultData(result));
    }
    async remove() {
        const [passed, causeOrBefore] = await this["replicaHelper"].checkRequestReplicaMap({ existent: true });
        if (passed !== true)
            return new tms_koa_1.ResultFault(causeOrBefore);
        return this["clReplicaMap"]
            .deleteOne({ _id: causeOrBefore._id })
            .then(() => new tms_koa_1.ResultData('ok'));
    }
    async synchronize() {
        const [passed, causeOrBefore] = await this["replicaHelper"].checkRequestReplicaMap({ existent: true });
        if (passed !== true)
            return new tms_koa_1.ResultFault(causeOrBefore);
        let { primary, secondary } = causeOrBefore;
        let result = await this["modelReplicaMap"].synchronize(primary, secondary);
        return new tms_koa_1.ResultData(result);
    }
    async synchronizeAll() {
        const count = await this["clReplicaMap"].countDocuments();
        if (count === 0)
            return new tms_koa_1.ResultFault('没有配置集合间复制关系，未执行集合同步操作');
        setTimeout(() => {
            logger.info(`启动后台执行集合复制关系同步count=${count}]`);
            const cp = require('child_process');
            const child = cp.fork('../replica/synchronize.js');
            child.on('message', (msg) => {
                logger.info(`结束后台执行集合复制关系同步[syncCount=${msg.syncCount}]`);
            });
        });
        return new tms_koa_1.ResultData(count);
    }
}
exports.ReplicaBase = ReplicaBase;
exports.default = ReplicaBase;
