...
| **ferretdb**                           |                                                                                                                           |                         |
| TMW_STORAGE_BACKEND                    | 存储后端模式，`mongodb`（默认）或 `ferretdb`。`ferretdb` 模式下 ACL/Tag/日志走 PG 直接查询，文档 CRUD 走 FerretDB 翻译层。  | mongodb                 |
| TMW_POSTGRES_URI                       | ferretdb 模式下 PG 连接地址，格式 `postgresql://user:pass@host:5432/db`。                                                    |                         |
| TMW_POSTGRES_MAX_POOL                  | ferretdb 模式下 PG 连接池最大连接数。                                                                                       | 10                      |
| **fs**                                 |                                                                                                                           |                         |
