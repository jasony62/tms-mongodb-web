...
let appConfig = {
  ...
  tmwConfig: {
    TMW_APP_DEFAULT_ACLCHECK_DB:
      /true|yes/i.test(env.TMW_APP_DEFAULT_ACLCHECK_DB) || false, // 数据库默认开启访问控制？
    TMW_APP_DEFAULT_ACLCHECK_CL:
      /true|yes/i.test(env.TMW_APP_DEFAULT_ACLCHECK_CL) || false, // 集合默认开启访问控制？
    TMW_APP_DEFAULT_ACLCHECK_DOC:
    ...
    TMW_APP_CREATOR: env.TMW_APP_CREATOR || 'creator', // 集合中添加、创建、导入数据时默认添加创建时间字段，字段名
