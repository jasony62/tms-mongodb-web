<template>
  ...
  >
    <el-table :data="tels" stripe ref="multipleTable" style="width: 100%" >
      <el-table-column
...
export default {
  ...
  methods: {
    confirm() {
      this.$emit('submit', this.tels)
    },
    open(tels, collection) {
      this.tels = tels
      this.collection = collection
      this.page.total = tels.length
      this.$mount()
