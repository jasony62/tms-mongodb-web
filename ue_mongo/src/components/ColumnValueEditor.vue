  ...
  <el-dialog :visible.sync="dialogVisible" :destroy-on-close="destroyOnClose" :close-on-click-modal="closeOnClickModal">
    <el-input placeholder="自定义选中列的值，不填则值为空" v-model="input" class="input-with-select">
      <el-select v-model="select" slot="prepend" placeholder="选择列" @change="handleSelect" clearable filterable>
        <el-option v-for="(s, k) in collection.schema.body.properties" :key="k" :prop="k" :label="s.title"
          :value="k"></el-option>
      </el-select>
    </el-input>
    <div style="margin: 10px 0" v-if="select &&
      ...
      <span style="margin-right: 20px" v-for="(s, k) in collection.schema.body.properties[select].enum" :key="k">{{
        s.label }}</span>
    </div>
    <div class="tmw-tags">
      <el-tag v-for="tag in tags" :key="tag.id" closable :disable-transitions="false" @close="removeTag(tag)">{{ tag.label
      }}:{{ tag.value }}</el-tag>
    </div>
...
import utils from '../tms/utils.js'
